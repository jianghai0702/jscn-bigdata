...
#加载数据到目标表中
#su - hdfs <<EOF
#/usr/bin/sqoop  --options-file /AiInsight/sqoop/hive_import_swj.txt --query 'select T.* from (
...
#) T
#where \$CONDITIONS' --target-dir "/asiainfo/hive/STG/STG_SZZG_ACCT_ITEM_N/pt_mon=${etl_cycle} " --hive-table  ${target_database}.${target_table_name} --hive-partition-key pt_mon --hive-partition-value "${etl_cycle}"
#
#
