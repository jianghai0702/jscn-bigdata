...
#目标表名
target_table_name="stg_ins_prod_res"
#源数据库表名（必须大写）
source_table_name="SO1.INS_PROD_RES"
#######################################################
#脚本开始
##########################################g#############
#运行日期
...
#加载数据到目标表中
su - hdfs <<EOF
/usr/bin/sqoop  --options-file /AiInsight/sqoop/hive_import.txt -m 4 --table ${source_table_name}  --hive-table  ${target_database}.${target_table_name} --hive-partition-key pt_mon --hive-partition-value "${etl_cycle}" 
