...
#-------------------------------------------------------------------------------
# Name: 
# Pay: st_diff_region_user_dev_m.sh
# Purpose:
#
# Author:      yanal
#
...
#------------------------------------------------------------------------------
#本表名
table_name="st_diff_region_user_dev_m"
##ST导出文件目录
#-------------------------------------------------------------------------------
#运行日期
...
select
    ...
    when to_char('TIMESTAMP(6)') then column_name||','
    when to_char('DATE') then column_name||','
...
#--------------------------------------------------------------------------------
#导出CSV文件
HIVE_STRING="hive -e \"use default;select * from "$table_name" where pt_mon='${lmonth}'\" > "$hadoop_output/$table_name".csv"
echo "$HIVE_STRING"
eval "$HIVE_STRING"
#---------------------------------------------------------------------------------
#删除原有数据
...
set serveroutput on
set timing on
delete from $table_name where cycle_id='${lmonth}';
commit;
EOF
#---------------------------------------------------------------------------------
#导入到ORACLE
