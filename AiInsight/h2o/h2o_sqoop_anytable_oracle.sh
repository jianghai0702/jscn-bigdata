...
hive_table_name="${1}"
#rdbms目标表名 
# 表名转换成大写，oracle要求表名为大写
rdbms_table_name="$(echo "${2}" | tr [a-z] [A-Z])"
...
#su - hdfs <<EOF
sqoop_str="/usr/bin/sqoop  --options-file /AiInsight/sqoop/hive_export_oracle.txt  --table "${rdbms_table_name}"  --hcatalog-database default --hcatalog-table "${hive_table_name}" "${partition_args}""
