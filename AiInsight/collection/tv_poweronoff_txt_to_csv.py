...
def csv_from_txt():
    try:
        source_dir='/home/ftpuser/tv/tv_pb_poweronoff_info/'
        in_date = sys.argv[1]
        tar_dir = "/data/tv_data/stg_tv_pb_poweronoff_info/"               #目标文件夹
        for root,sub_dirs,files in os.walk(source_dir):
           for txt in files:                                     #将目录下文件名存入队列，循环读取队列
             if re.findall(r'pb_poweronoff_info(\d{8})\.txt',txt)[0] != str(in_date):
                 continue
