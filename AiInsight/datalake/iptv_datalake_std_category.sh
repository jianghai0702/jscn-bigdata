...
# 源数据库
###############
dbTableName="dim_std_category"
targetDir="/asiainfo/${siteName}/datasource/${dbTableName}/${pt_mon}"
...
su - hdfs <<EOF
hive -e """
CREATE DATABASE IF NOT EXISTS ${siteName};
"""
exit;
EOF
...
su - hdfs <<EOF
hive -e """
CREATE TABLE IF NOT EXISTS ${siteName}.${dbTableName} (
channel_name string,
...
lines terminated by '\n'
stored as textfile
LOCATION '/asiainfo/${siteName}/datalake/${dbTableName}';
"""
exit;
EOF
...
su - hdfs <<EOF
hive -e """
LOAD DATA INPATH '${targetDir}' OVERWRITE INTO TABLE  ${siteName}.${dbTableName} PARTITION (pt_mon='${pt_mon}');
"""
exit;
EOF
