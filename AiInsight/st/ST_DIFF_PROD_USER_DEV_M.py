...
#-------------------------------------------------------------------------------
# Name: ST用户发展按产品统计(月)
# Pay: ST_DIFF_PROD_USER_DEV_M.py
# Purpose:
#
# Author:      liuzx
#
...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_prod_user_dev_m      				    =    "st_diff_prod_user_dev_m"
    t_st_diff_prod_user_dev_m_tmp  				    =    "st_diff_prod_user_dev_m_tmp"
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#用来存放中间结果数据，当月数据
    hsql='''create table if not exists %(t_st_diff_prod_user_dev_m_tmp)s
            (
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(t_st_diff_prod_user_dev_m_tmp)s'
            '''% vars()    
...
##===========================================================================
#生成临时表
    hsql='''insert overwrite table %(t_st_diff_prod_user_dev_m_tmp)s
            select
              ...
              '' area_std_addr_id,
              '' area_std_addr_name,
              t3.product_id std_up_product_code,
              t1.srvpkg_name std_up_product_name,
            ...
            group by t1.own_corp_std_org_code,t2.std_org_name,t1.management_station,t3.product_id,t1.srvpkg_name
            '''% vars()
    QueryExe(hsql,name,dates)

#生成目标表数据
    hsql='''insert overwrite table %(t_st_diff_prod_user_dev_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
              ...
              t1.repay_user_count,
              t1.repay_user_count+nvl(t2.cum_repay_user_count,0) cum_repay_user_count,
              from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date,
              t1.user_count
            from %(t_st_diff_prod_user_dev_m_tmp)s t1
            left join %(t_st_diff_prod_user_dev_m)s t2 on t1.own_corp_std_org_code=t2.own_corp_std_org_code
                                                          ...
                                                          and t1.std_up_product_code=t2.std_up_product_code
                                                          and t1.std_up_product_name=t2.std_up_product_name
...
##删除临时表                                               
##===========================================================================
#    hsql='''drop table %(t_st_diff_prod_user_dev_m_tmp)s''' %vars()
#    QueryExe(hsql,name,dates)
