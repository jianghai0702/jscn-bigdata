...
#
    ...
    hsql='''insert overwrite table %(t_st_arpu)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select t.cycle_id
                   ,t.corp_org_name
                   ,t.kd_arpu
                   ,t.sz_arpu
                   ,t.kd_arpu+t.sz_arpu
            from %(s_stg_arpu)s t
            where t.cycle_id = '%(ARG_OPTIME_LAST1_M)s'
               and t.pt_mon = %(ARG_OPTIME_LAST1_M)s
            '''% vars()
