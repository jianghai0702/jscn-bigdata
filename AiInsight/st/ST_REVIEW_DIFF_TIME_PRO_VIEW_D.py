...
## -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name: ST不同时段不同节目回看收视情况(日)
# Pay: ST_REVIEW_DIFF_TIME_PRO_VIEW_D.py
# Purpose:
#
# Author:    lim,liuzx
#
...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_review_diff_time_pro_view_d      =    "st_review_diff_time_pro_view_d"
    t_st_review_diff_time_pro_view_d_tmp1 =    "st_review_diff_time_pro_view_d_tmp1"
    t_st_review_diff_time_pro_view_d_tmp2 =    "st_review_diff_time_pro_view_d_tmp2"
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#用来存放回看的清洗数据，包括时段开始时间结束时间的时间戳
    hsql='''create table if not exists %(t_st_review_diff_time_pro_view_d_tmp1)s
            (
        id                  string,
        ...
        end_time            int
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(t_st_review_diff_time_pro_view_d_tmp1)s'
            '''% vars()    
    QueryExe(hsql,name,dates)

#用来存放标准时段的清洗数据，包括时段开始时间结束时间的时间戳
    hsql='''create table if not exists %(t_st_review_diff_time_pro_view_d_tmp2)s
            (
        ...
        end_time           int
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(t_st_review_diff_time_pro_view_d_tmp2)s'
            '''% vars()    
    QueryExe(hsql,name,dates)
...
#上周的日期列表yyyymmdd
    ...
    for n in range(1,len(li)+1):
      ...
      #在临时表1中删除变更数据
      hsql='''insert overwrite table %(t_st_review_diff_time_pro_view_d_tmp1)s
              select
                t1.user_id id,
                t2.std_program_code,
      ...
      #在临时表2中删除变更数据
      hsql='''insert overwrite table %(t_st_review_diff_time_pro_view_d_tmp2)s
              select
                t1.std_times_code,
                t1.std_times_name,
                t1.min_value,
      ...
      #插入当前dwd全量数据
      hsql='''insert overwrite table %(t_st_review_diff_time_pro_view_d)s partition(pt_time=%(v_date)s)
              select
                %(v_date)s cycle_id,
                ...
                t2.std_times_code,
                t2.std_times_name,
                count(distinct t1.id) review_user_count,
                count(*) review_count,
                ...
                round(sum(if(t1.end_time<t2.end_time,t1.end_time,t2.end_time)
                         -if(t1.start_time>t2.start_time,t1.start_time,t2.start_time))
                     /count(distinct t1.id)/60,2) avg_review_time,
                from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date
              from %(t_st_review_diff_time_pro_view_d_tmp1)s t1,%(t_st_review_diff_time_pro_view_d_tmp2)s t2
