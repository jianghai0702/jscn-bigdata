...
##===========================================================================     
    ...
    s_st_user_dev_m                   =    "st_user_dev_m"
    s_dim_std_product                 =    "dim_std_product"
    s_dim_std_organize                =    "dim_std_organize"
    s_temp_st_income_m1               =    "temp_st_income_m1"
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_income_m                =    "st_income_m"      
##===========================================================================       
##创建临时表                                                        
##===========================================================================
#存放    新订购用户数,新订购收入,销账用户数,销账收入,宽带伴侣点播收入
    hsql='''create table if not exists %(s_temp_st_income_m1)s
            (
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_income_m1)s'
            '''% vars()    
...
#生成     新订购用户数,新订购收入,平均每个月订购收入
    hsql='''insert overwrite table %(s_temp_st_income_m1)s
            select
             ...
             group by t1.own_corp_std_org_code,t3.std_org_name,t1.management_station
          '''% vars()    
    QueryExe(hsql,name,dates)

#生成     销账用户数,销账收入
    hsql='''insert into table %(s_temp_st_income_m1)s
            select
                t1.own_corp_std_org_code,
                t3.std_org_name own_corp_std_org_name,
                '' mgr_addr_id,
             ...
             left outer join %(s_dim_std_organize)s t3 on t1.own_corp_std_org_code=t3.std_org_code
             where t1.pt_mon=%(ARG_OPTIME_LAST1_M)s
             group by t1.own_corp_std_org_code,t3.std_org_name,t2.management_station
          '''% vars()    
    QueryExe(hsql,name,dates)

#生成      新订购用户数,新订购收入,销账用户数,销账收入,宽带伴侣点播收入 汇总数据
    hsql='''insert overwrite table %(s_temp_st_income_m1)s
            select
                t1.own_corp_std_org_code,
...
##===========================================================================
#生成    当月的记录；
    hsql='''insert overwrite table %(t_st_income_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
                ...
                t1.new_order_amount,
                case when coalesce(t2.new_order_amount,0)=0 then 0 else round(t1.new_order_amount/t2.new_order_amount,2) end new_order_amount_chain,
                case when coalesce(t3.new_order_amount,0)=0 then 0 else round(t1.new_order_amount/t3.new_order_amount,2) end new_order_amount_ear,
                ...
                t1.bill_user_count,
                t1.bill_amount,
                case when coalesce(t2.bill_amount,0)=0 then 0 else round(t1.bill_amount/t2.bill_amount,2) end bill_amount_chain,
                case when coalesce(t3.bill_amount,0)=0 then 0 else round(t1.bill_amount/t3.bill_amount,2) end bill_amount_ear,
                case when coalesce(t4.payment_user_count,0)=0 then 0 else round(t1.bill_amount/t4.payment_user_count,2) end bill_amount_arpu,
                t1.lan_partner_amount,
                from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date
             from %(s_temp_st_income_m1)s t1
             left outer join %(t_st_income_m)s t2 on t1.own_corp_std_org_code=t2.own_corp_std_org_code 
             ...
             left outer join %(t_st_income_m)s t3 on t1.own_corp_std_org_code=t3.own_corp_std_org_code 
             ...
             left outer join %(s_st_user_dev_m)s t4 on t1.own_corp_std_org_code=t4.own_corp_std_org_code 
                                                     and t1.own_corp_std_org_name=t4.own_corp_std_org_name
                                                     and t1.mgr_addr_id=t4.mgr_addr_id
                                                     and t1.mgr_addr_name=t4.mgr_addr_name
                                                     and t1.community_id=t4.community_id
                                                     and t1.community_name=t4.community_name
                                                     and t1.area_std_addr_id=t4.area_std_addr_id
                                                     and t1.area_std_addr_name=t4.area_std_addr_name
...
##删除临时表                                               
##===========================================================================
#    hsql='''drop table %(s_temp_st_income_m1)s''' %vars()
#    QueryExe(hsql,name,dates)
