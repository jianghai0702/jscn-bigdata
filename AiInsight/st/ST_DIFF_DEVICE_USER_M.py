...
##自定义变量声明---源表声明                                                          
##===========================================================================
    s_dwa_diff_organize_user_m                =    "dwa_diff_organize_user_m"
    s_dwa_diff_online_media_user_d              =    "dwa_diff_online_media_user_d"  
    s_dim_std_organize                            =     'dim_std_organize'
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_device_user_m              =    "st_diff_device_user_m"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#ST不同设备访问用户
    hsql='''insert overwrite table %(t_st_diff_device_user_m)s partition(pt_mon = %(ARG_OPTIME_LAST1_M)s)
    select
       coalesce(t1.own_corp_std_org_code,t4.std_org_code,'10')   as OWN_CORP_STD_ORG_CODE    --标准归属分公司编码
       ,coalesce(t1.own_corp_std_org_name,t4.std_org_name,'江苏有线')  as OWN_CORP_STD_ORG_NAME    --标准归属分公司名称
       ,t2.STD_DEVICE_CATE_CODE     --标准设备类别编码 
       ,t2.STD_DEVICE_CATE_NAME     --标准设备类别名称 
       ,count(1) VISIT_COUNT              --访问次数          
       ,count(distinct t2.login_name) VISIT_USER_COUNT         --访问用户数   
       ,from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date  
    from %(s_dwa_diff_organize_user_m)s t1 
        right join %(s_dwa_diff_online_media_user_d)s t2 --在线媒体用户
    ...
    left join %(s_dim_std_organize)s t4
        on t4.std_org_code = t3.p_std_org_code
        where t1.pt_mon = %(ARG_OPTIME_LAST1_M)s
        and t2.pt_time like '%(ARG_OPTIME_LAST1_M)s%%'
        group by own_corp_std_org_code,OWN_CORP_STD_ORG_NAME,t4.std_org_code,t4.std_org_name,STD_DEVICE_CATE_CODE,STD_DEVICE_CATE_NAME
    grouping sets ( (own_corp_std_org_code,OWN_CORP_STD_ORG_NAME,t4.std_org_code,t4.std_org_name,STD_DEVICE_CATE_CODE,STD_DEVICE_CATE_NAME),
    (STD_DEVICE_CATE_CODE,STD_DEVICE_CATE_NAME,t4.std_org_code,t4.std_org_name),
    (STD_DEVICE_CATE_CODE,STD_DEVICE_CATE_NAME)
    ) 
     '''% vars()
