...
##===========================================================================     
    ...
    s_temp_st_diff_corp_income_m1     =    "temp_st_diff_corp_income_m1"
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_corp_income_m                =    "st_diff_corp_income_m"      
##===========================================================================       
##创建临时表                                                        
##===========================================================================
#存放    新订购用户数,新订购非0元用户数,新订购收入,平均每个月新订购收入,销账用户数,销账收入,宽带伴侣点播收入,缴费用户数
    hsql='''create table if not exists %(s_temp_st_diff_corp_income_m1)s
            (
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_diff_corp_income_m1)s'
            '''% vars()    
...
#生成     新订购用户数,新订购非0元用户数,新订购收入,平均每个月新订购收入
    hsql='''insert overwrite table %(s_temp_st_diff_corp_income_m1)s
            select
            ...
            group by t1.own_corp_std_org_code,t3.std_org_name
          '''% vars()    
    QueryExe(hsql,name,dates)
    
#生成  用户状态正常，订购时间小于数据日期，订购的产品是包月产品的收入（这种情况也算为“新订购收入”）
    hsql='''insert into table %(s_temp_st_diff_corp_income_m1)s
            select
                t1.own_corp_std_org_code,
                t3.std_org_name own_corp_std_org_name,
...
#生成     销账用户数,销账收入
    hsql='''insert into table %(s_temp_st_diff_corp_income_m1)s
            ...
            left outer join %(s_dim_std_organize)s t2 on t1.own_corp_std_org_code=t2.std_org_code
            where t1.pt_mon=%(ARG_OPTIME_LAST1_M)s
...
#生成      新订购用户数,新订购收入,销账用户数,销账收入,宽带伴侣点播收入,缴费用户数 汇总数据
    hsql='''insert overwrite table %(s_temp_st_diff_corp_income_m1)s
            select
                t1.own_corp_std_org_code,
                ...
                sum(t1.lan_partner_amount) lan_partner_amount,
                sum(t1.payment_user_count) payment_user_count
            from %(s_temp_st_diff_corp_income_m1)s t1
            group by t1.own_corp_std_org_code,t1.own_corp_std_org_name
          '''% vars()    
    QueryExe(hsql,name,dates)
...
##===========================================================================
#生成    当月的记录；
    hsql='''insert overwrite table %(t_st_diff_corp_income_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
                ...
                case when coalesce(t2.new_order_amount,0)=0 then 0 else round(t1.new_order_amount/t2.new_order_amount*100,2) end new_order_amount_chain,
                case when coalesce(t3.new_order_amount,0)=0 then 0 else round(t1.new_order_amount/t3.new_order_amount*100,2) end new_order_amount_ear,
                case when coalesce(t2.new_order_amount,0)=0 then 0 else round((t1.new_order_amount-t2.new_order_amount)/t2.new_order_amount*100,2) end new_order_amount_chain_growth,
                case when coalesce(t3.new_order_amount,0)=0 then 0 else round((t1.new_order_amount-t3.new_order_amount)/t3.new_order_amount*100,2) end new_order_amount_ear_growth,
                ...
                case when coalesce(t2.bill_amount,0)=0 then 0 else round(t1.bill_amount/t2.bill_amount*100,2) end bill_amount_chain,
                case when coalesce(t3.bill_amount,0)=0 then 0 else round(t1.bill_amount/t3.bill_amount*100,2) end bill_amount_ear,
                ...
                round(t1.lan_partner_amount,2) lan_partner_amount,
                from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date
            from %(s_temp_st_diff_corp_income_m1)s t1
            left outer join %(t_st_diff_corp_income_m)s t2 on t1.own_corp_std_org_code=t2.own_corp_std_org_code 
                                                              and t1.own_corp_std_org_name=t2.own_corp_std_org_name
                                                              and t2.pt_mon=%(ARG_OPTIME_LAST2_M)s
            left outer join %(t_st_diff_corp_income_m)s t3 on t1.own_corp_std_org_code=t3.own_corp_std_org_code 
...
##删除临时表                                               
##===========================================================================
#    hsql='''drop table %(s_temp_st_diff_corp_income_m1)s''' %vars()
#    QueryExe(hsql,name,dates)
