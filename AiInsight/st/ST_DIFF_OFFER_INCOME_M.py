...
##===========================================================================     
    ...
    s_temp_st_diff_offer_income_m1    =    "temp_st_diff_offer_income_m1"
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_offer_income_m                =    "st_diff_offer_income_m"      
##===========================================================================       
##创建临时表                                                        
##===========================================================================
#存放    新订购用户数,新订购非0元用户数,新订购收入,平均每个月新订购收入,销账用户数,销账收入,缴费用户数
    hsql='''create table if not exists %(s_temp_st_diff_offer_income_m1)s
            (
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_diff_offer_income_m1)s'
            '''% vars()    
...
#生成     新订购用户数,新订购非0元用户数,新订购收入,平均每个月新订购收入
    hsql='''insert overwrite table %(s_temp_st_diff_offer_income_m1)s
            select
                t1.own_corp_std_org_code,
                t3.std_org_name own_corp_std_org_name,
                '' std_offer_code,
                t2.offer_name std_offer_name,
            ...
            group by t1.own_corp_std_org_code,t3.std_org_name,t2.offer_name
          '''% vars()    
    QueryExe(hsql,name,dates)
    
#生成  用户状态正常，订购时间小于数据日期，订购的产品是包月产品的收入（这种情况也算为“新订购收入”）
    hsql='''insert into table %(s_temp_st_diff_offer_income_m1)s
            select
                t1.own_corp_std_org_code,
                t3.std_org_name own_corp_std_org_name,
                '' std_offer_code,
...
#生成     销账用户数,销账收入
    hsql='''insert into table %(s_temp_st_diff_offer_income_m1)s
            ...
            where t1.pt_mon=%(ARG_OPTIME_LAST1_M)s
            group by t1.own_corp_std_org_code,t3.std_org_name,t4.offer_name
          '''% vars()    
    QueryExe(hsql,name,dates)
...
#生成      新订购用户数,新订购收入,销账用户数,销账收入,缴费用户数 汇总数据
    hsql='''insert overwrite table %(s_temp_st_diff_offer_income_m1)s
            ...
            from %(s_temp_st_diff_offer_income_m1)s t1
            group by t1.own_corp_std_org_code,t1.own_corp_std_org_name,t1.std_offer_code,t1.std_offer_name
          '''% vars()    
    QueryExe(hsql,name,dates)
...
##===========================================================================
#生成    当月的记录；
    hsql='''insert overwrite table %(t_st_diff_offer_income_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
...
##删除临时表                                               
##===========================================================================
#    hsql='''drop table %(s_temp_st_diff_offer_income_m1)s''' %vars()
#    QueryExe(hsql,name,dates)
