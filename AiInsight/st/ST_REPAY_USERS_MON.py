...
#-------------------------------------------------------------------------------
# Name: ST续费用户情况（月）      
# Pay: ST_REPAY_USERS_MON.py
# Purpose: 这个表数据取本月续费用户的上月情况
# 
# Author:      yanal
#
# Created:     20-12-2016
...
# -------------------------------
# 由于这个表是要获取本月续费用户的上月情况，因此：用户表基础信息取本月数据；
...
##===========================================================================     
    ...
    s_temp_st_repay_users_mon1         =    "temp_st_repay_users_mon1"
    s_temp_st_repay_users_mon2         =    "temp_st_repay_users_mon2"
    s_temp_st_repay_users_mon3         =    "temp_st_repay_users_mon3"
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_repay_users_mon                =    "st_repay_users_mon"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
# 下行流量数据临时表
    hsql='''create table if not exists %(s_temp_st_repay_users_mon1)s  (
        login_name           string,
        month_up_flow        double,
        month_down_flow      double
    )
    STORED AS RCFILE
    LOCATION '%(tmp_path)s/%(s_temp_st_repay_users_mon1)s'
            '''% vars()
    QueryExe(hsql,name,dates)

# 认证次数临时表
    hsql='''create table if not exists %(s_temp_st_repay_users_mon2)s  (
        login_name           string,
        auth_count           int,
        auth_success_rate    double
    )
    STORED AS RCFILE
    LOCATION '%(tmp_path)s/%(s_temp_st_repay_users_mon2)s'
            '''% vars()
    QueryExe(hsql,name,dates)

# 获取上网天数和上网时长数据
    hsql='''create table if not exists %(s_temp_st_repay_users_mon3)s  (
        login_name           string,
    ...
    STORED AS RCFILE
    LOCATION '%(tmp_path)s/%(s_temp_st_repay_users_mon3)s'
            '''% vars()
    QueryExe(hsql,name,dates)
...
#从认证的在线记录表取得上下行流量数据 
    hsql='''insert overwrite table %(s_temp_st_repay_users_mon1)s 
            select 
            t.login_name,
...
#从认证记录表取得 认证次数数据
    hsql='''insert overwrite table %(s_temp_st_repay_users_mon2)s 
            select 
            t.login_name,
...
# 取得本月续费用户 上月有效上网次数和总时长数据
    hsql='''insert overwrite table %(s_temp_st_repay_users_mon3)s 
            select distinct
            t1.login_name           ,
            ...
            where pt_mon=%(ARG_OPTIME_LAST1_M)s and is_this_repay=1
            ) t
            join 
...
# 生成目标表数据
    hsql='''insert overwrite table %(t_st_repay_users_mon)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select 
            ...
            left join %(s_temp_st_repay_users_mon1)s t2 on t.login_name=t2.login_name
            left join %(s_temp_st_repay_users_mon2)s t3 on t.login_name=t3.login_name 
            left join %(s_temp_st_repay_users_mon3)s t4 on t.login_name=t4.login_name
