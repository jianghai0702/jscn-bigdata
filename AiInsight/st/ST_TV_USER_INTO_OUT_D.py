...
# -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name:ST直播各频道用户流入流出情况(日)
# Pay:ST_TV_USER_INTO_OUT_D.py
# Purpose:
#
# Author:      chens
#
...
##===========================================================================     
    ...
    s_temp_st_tv_user_into_out_d1    =    "temp_st_tv_user_into_out_d1"
    s_temp_st_tv_user_into_out_d2    =    "temp_st_tv_user_into_out_d2"
    s_temp_st_tv_user_into_out_d3    =    "temp_st_tv_user_into_out_d3"
    s_temp_st_tv_user_into_out_d4    =    "temp_st_tv_user_into_out_d4"
    s_temp_st_tv_user_into_out_d5    =    "temp_st_tv_user_into_out_d5"
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_tv_user_into_out_d                =    "st_tv_user_into_out_d"      
##===========================================================================       
##创建临时表                                                        
##===========================================================================
##===========================================================================       
...
#用来存放直播的清洗数据，包括频道，开始结束时间的时间戳等，并排除持续时间小于等于10秒的数据,然后按用户分组，时间降序排序；
    hsql='''create table if not exists %(s_temp_st_tv_user_into_out_d1)s
            (
              serial_id       string,
              begin_time      string,
              end_time        string,
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_tv_user_into_out_d1)s'
            '''% vars()    
    QueryExe(hsql,name,dates)

#存放  同一个用户从一个频道换到另一个频道的记录
    hsql='''create table if not exists %(s_temp_st_tv_user_into_out_d2)s
            (
              serial_id            string,
              begin_time           string,
              end_time             string,
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_tv_user_into_out_d2)s'
            '''% vars()    
    QueryExe(hsql,name,dates)

#用来存放标准时段的清洗数据，包括时段开始时间结束时间的时间戳
    hsql='''create table if not exists %(s_temp_st_tv_user_into_out_d3)s
            (
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_tv_user_into_out_d3)s'
            '''% vars()    
    QueryExe(hsql,name,dates)

#用来存放关联标准时段之后的数据
    hsql='''create table if not exists %(s_temp_st_tv_user_into_out_d4)s
            (
              serial_id            string,
              channel_name         string,
              begin_time           string,
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_tv_user_into_out_d4)s'
            '''% vars()    
    QueryExe(hsql,name,dates)
    
#存放    各频道用户流入流出情况
    hsql='''create table if not exists %(s_temp_st_tv_user_into_out_d5)s
            (
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_temp_st_tv_user_into_out_d5)s'
            '''% vars()    
    QueryExe(hsql,name,dates)
...
#生成    包括频道，开始结束时间的时间戳等，并排除持续时间小于等于10秒的数据
    hsql='''insert overwrite table %(s_temp_st_tv_user_into_out_d1)s
            select
                t1.user_id serial_id,
                t1.start_time,
                t1.end_time,
                t1.event_desc channel_name,
                ...
                row_number() over(distribute by t1.user_id sort by t1.begin_time) rn
...
#生成    同一个用户从一个频道换到另一个频道的记录
    hsql='''insert overwrite table %(s_temp_st_tv_user_into_out_d2)s
            select
                t1.serial_id,
                t1.begin_time,
                t1.end_time,
             ...
             from %(s_temp_st_tv_user_into_out_d1)s t1
             left outer join %(s_temp_st_tv_user_into_out_d1)s t2 on t1.serial_id=t2.serial_id and t1.rn+1=t2.rn
          '''% vars()    
    QueryExe(hsql,name,dates)

#生成   标准时段的清洗数据，包括时段开始时间结束时间的时间戳
    hsql='''insert overwrite table %(s_temp_st_tv_user_into_out_d3)s
            select
                t1.std_times_code,
                t1.std_times_name,
                t1.min_value,
...
#排除同一个用户中，下一个频道开始时间-前一个频道的结束时间大于>=5分钟的记录，防止关机后过了一段时间再开机的情况
    hsql='''insert overwrite table %(s_temp_st_tv_user_into_out_d4)s
            select
                t1.serial_id,
                t1.channel_name,
                t1.begin_time,
                ...
                t2.std_times_code,
                t2.std_times_name
            from %(s_temp_st_tv_user_into_out_d2)s t1,%(s_temp_st_tv_user_into_out_d3)s t2
...
#生成    用户流入情况；
    hsql='''insert overwrite table %(s_temp_st_tv_user_into_out_d5)s
            select
                ...
                0 out_user_count
             from %(s_temp_st_tv_user_into_out_d4)s t1
             left outer join %(s_dim_std_channel)s t2 on t1.next_channel_name=t2.std_channel_name
             group by t1.std_times_code,t1.std_times_name,t2.std_channel_code,t1.next_channel_name
          '''% vars()    
    QueryExe(hsql,name,dates)
    
#生成    用户流出情况；
    hsql='''insert into table %(s_temp_st_tv_user_into_out_d5)s
            select
                '' own_corp_std_org_code,
                '' own_corp_std_org_name,
                t1.std_times_code,
             ...
             group by t1.std_times_code,t1.std_times_name,t2.std_channel_code,t1.channel_name
          '''% vars()    
    QueryExe(hsql,name,dates)
...
##===========================================================================
#生成    当天的记录；
    hsql='''insert overwrite table %(t_st_tv_user_into_out_d)s partition(pt_time=%(ARG_OPTIME_LASTDAY)s)
            select
...
##删除临时表                                               
##===========================================================================
#    hsql='''drop table %(s_temp_st_tv_user_into_out_d1)s''' %vars()
#    QueryExe(hsql,name,dates)

#    hsql='''drop table %(s_temp_st_tv_user_into_out_d2)s''' %vars()
#    QueryExe(hsql,name,dates)

#    hsql='''drop table %(s_temp_st_tv_user_into_out_d3)s''' %vars()
#    QueryExe(hsql,name,dates)

#    hsql='''drop table %(s_temp_st_tv_user_into_out_d4)s''' %vars()
#    QueryExe(hsql,name,dates)

#    hsql='''drop table %(s_temp_st_tv_user_into_out_d5)s''' %vars()
#    QueryExe(hsql,name,dates)
