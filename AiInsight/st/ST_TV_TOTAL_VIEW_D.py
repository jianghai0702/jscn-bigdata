...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_tv_total_view_d                =    "st_tv_total_view_d"      
##===========================================================================       
##创建临时表                                                        
##===========================================================================
...
##===========================================================================
#生成    当月的记录；
    hsql='''insert overwrite table %(t_st_tv_total_view_d)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
                ...
                round(sum(t1.duration)/60,2) as tv_time,
             ...
             group by t1.tv_day,t1.own_corp_std_org_code,t2.std_org_name
          '''% vars()    
    QueryExe(hsql,name,dates)

# 向上钻取到地市、省汇总统计
    hsql='''insert into table %(t_st_tv_total_view_d)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select cycle_id,
            ...
            from %(t_st_tv_total_view_d)s as t1
                 LEFT JOIN %(s_dim_std_organize)s as t2
                     ON t1.own_corp_std_org_code = t2.std_org_code
                 LEFT JOIN %(s_dim_std_organize)s as t3
                     ON t3.std_org_code = t2.p_std_org_code
         WHERE t1.pt_mon = '%(ARG_OPTIME_LAST1_M)s'
         GROUP BY cycle_id,t3.std_org_code,t3.std_org_name
         GROUPING SETS ((cycle_id,t3.std_org_code,t3.std_org_name),cycle_id)
          '''% vars()    
    QueryExe(hsql,name,dates)
