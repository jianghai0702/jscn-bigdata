...
#-------------------------------------------------------------------------------
# Name:ST不同套餐不同网站类型用户
# Pay:ST_DIFF_OFFER_URLTYPE_USER_M.py
# Purpose: 
#
# Author:      zhangzc
#
...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_offer_urltype_user_m                =    "st_diff_offer_urltype_user_m"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#ST不同套餐不同网站类型用户
    hsql='''insert overwrite table %(t_st_diff_offer_urltype_user_m)s partition(pt_mon = %(ARG_OPTIME_LAST_M)s)
    select
        t1.OWN_CORP_STD_ORG_CODE    --标准归属分公司编码 
        ,t1.OWN_CORP_STD_ORG_NAME   --标准归属分公司名称 
        ,'' as STD_OFFER_CODE          --标准套餐编码       
        ,t3.OFFER_NAME as STD_OFFER_NAME          --标准套餐名称       
        ,t4.URL_TYPE                --网址类型           
    ...
    from %(s_dwa_diff_organize_user_m)s t1 
        ...
        left join %(s_dim_std_product)s t3
        on t1.OWN_CORP_STD_ORG_CODE = t3.OWN_CORP_STD_ORG_CODE
        left join %(s_dim_std_url)s t4
        on t2.std_online_media_code = t4.STD_URL_CODE
        where t1.pt_mon = %(ARG_OPTIME_LAST_M)s
        and t2.pt_time like '%(ARG_OPTIME_LAST_M)s%%'
        group by t1.own_corp_std_org_code,OWN_CORP_STD_ORG_NAME,t3.OFFER_NAME,t4.URL_TYPE 
    '''% vars()
