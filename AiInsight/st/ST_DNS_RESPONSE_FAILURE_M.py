...
## -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name:ST_DNS响应失败情况分析(月)
# Pay:ST_DNS_RESPONSE_FAILURE_M.py
# Purpose: 
#
# Author:      liuyf7
#
...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_dns_response_failure_m              =    "st_dns_response_failure_m"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#下行数据中应答代码不为零
    hsql='''insert overwrite table %(t_st_dns_response_failure_m)s partition(pt_mon = %(ARG_OPTIME_LAST_M)s)
    select
...
#上下行条数差异
    hsql='''insert into table %(t_st_dns_response_failure_m)s partition(pt_mon = %(ARG_OPTIME_LAST_M)s)
select 
        '10'    --标准归属分公司编码
       ,'江苏有线'    --标准归属分公司名称
       ,'上行减下行差异次数' ANSWER_CODE     --应答代码 
       ,'' ANSWER_NAME     --应答含义 
       ,t1.count-t2.count RESPONSE_COUNT              --失败次数
       ,from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date  
from (select count(1) as count from %(s_dwd_dns_up_stream)s where substr(pt_time,1,6) = %(ARG_OPTIME_LAST1_M)s and pt_time >20170802 and pt_time<20170809) t1 --上行
    ,(select count(1) as count from %(s_dwd_dns_down_stream)s where substr(pt_time,1,6) = %(ARG_OPTIME_LAST1_M)s and pt_time >20170802 and pt_time<20170809) t2 --下行
