...
# -------------------------------
# 由于这个表是要获取:上个月到期，本月未续费-上月情况，因此：用户表基础信息取本月数据；
...
# -----------------------------
# wangxx 2017-9-22
# 修改流失用户判断逻辑，上上个月到期，上个月没续费的用户，算流失用户
#-------------------------------------------------------------------------------
...
# 生成目标表数据
    hsql='''insert overwrite table %(t_st_lost_users_mon)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            ...
            (
             ...
             where pt_mon=%(ARG_OPTIME_LAST1_M)s  and  status_name = '停机' and order_priority = '1'  and  substr(expire_date,1,7) = '%(ARG_OPTIME_LAST3_M_ISO)s'
            )t
