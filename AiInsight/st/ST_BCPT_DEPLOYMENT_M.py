...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_bcpt_deployment_m        				    =    "st_bcpt_deployment_m"
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
...
##===========================================================================
#插入网络环境数据
    hsql='''insert overwrite table %(t_st_bcpt_deployment_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
    		      %(ARG_OPTIME_LAST1_M)s cycle_id,
    		      '网络环境' data_source,
    		      t1.monitor_city region_name,
    		      count(distinct t1.monitor_ip) monitor_num,
    		      count(distinct t1.main_id) main_id_count,
    		      count(distinct t1.increment_id) increment_id_count,
              from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date
            from %(s_dwd_network)s t1
...
#插入流媒体数据
    hsql='''insert into table %(t_st_bcpt_deployment_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
    		      %(ARG_OPTIME_LAST1_M)s cycle_id,
    		      '流媒体' data_source,
    		      t1.monitor_city region_name,
