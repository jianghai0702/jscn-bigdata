...
## -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name: ST各时段的上网用户数统计(月)
# Pay: ST_DIFF_TIMES_USER_M.py
# Purpose:
#
# Author:      liuyf7
#
...
##===========================================================================
    ...
    s_dwa_diff_times_user_d                     =    "dwa_diff_times_user_d"
    s_dim_std_organize                          =    "dim_std_organize"
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_times_user_m     =    "st_diff_times_user_m"
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
...
#生成目标表数据
    hsql='''insert overwrite table %(t_st_diff_times_user_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
              %(ARG_OPTIME_LAST1_M)s cycle_id,
              t2.std_times_code,
              t2.std_times_name,
              t2.user_count,
              if(coalesce(t1.total_user_count,0)=0,0,round(t2.user_count/t1.total_user_count*100,2)),
              from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date,
              t1.own_corp_std_org_code,
              t4.std_org_name
            from (--总付费用户数
                  select own_corp_std_org_code,count(distinct prod_inst_id) total_user_count 
                    from %(s_dwa_fact_rpt_ins_prod_mon)s
                   where is_valid = 1
                     and pt_mon = %(ARG_OPTIME_LAST1_M)s
                   group by own_corp_std_org_code
                  ) t1
            inner join (--按时段分组，统计用户数
                 select std_times_code,
                        std_times_name,
                        b.own_corp_std_org_code,
                        count(distinct a.login_name) user_count
                   from %(s_dwa_diff_times_user_d)s a
                  inner join %(s_dwa_fact_rpt_ins_prod_mon)s b
                     on a.login_name = b.login_name
                    and b.is_valid = 1
                    and b.pt_mon = %(ARG_OPTIME_LAST1_M)s
                  where substr(a.pt_time,1,6) = %(ARG_OPTIME_LAST1_M)s
                  group by std_times_code,std_times_name,own_corp_std_org_code
