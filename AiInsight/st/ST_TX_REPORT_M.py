...
#===========================================================================================
    ...
    ARG_OPTIME_LAST2_MEND     = dicts['ARG_OPTIME_LAST2_MEND']        #获得传入的数据日期的上两月最后一天日期yyyy-mm-dd格式的数据日期
...
##===========================================================================
    ...
    s_st_tx_report_m1          =    "s_st_tx_report_m1"
    s_st_tx_report_m2          =    "s_st_tx_report_m2"
##===========================================================================
##自定义变量声明---目标表声明
##===========================================================================
    t_st_tx_report_m     =    "st_tx_report_m"
##===========================================================================
##创建临时表
##===========================================================================
#用来存放中间结果数据，本月用户登录次数
    hsql='''create table if not exists %(s_st_tx_report_m1)s
            (
                owner_id    string,
                login_count int
            )
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_st_tx_report_m1)s'
            '''% vars()    
...
##===========================================================================
#生成用户本月登录次数
    hsql='''insert overwrite table %(s_st_tx_report_m1)s
            select t1.row_id
            ...
            left join %(s_dwd_visqual_probes)s t2
             ...
             and t2.pt_time = %(ARG_OPTIME_LAST2_MEND)s
           where t1.pt_time = %(ARG_OPTIME_LASTMONEND)s
            '''% vars()
    QueryExe(hsql,name,dates)

#生成目标表数据
    hsql='''insert overwrite table %(t_st_tx_report_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
              %(ARG_OPTIME_LAST1_M)s cycle_id,
              active_user_count, --月活跃用户
              case when total_user_count = 0 then 0 else round(active_user_count/total_user_count*100,2) end as active_rate, --月活跃度
              from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date
            from(select count(*) total_user_count --总用户数
                   from %(s_dwd_probe_setup_log)s t2--用户
                  where t2.pt_time = %(ARG_OPTIME_LASTMONEND)s
                ) t2,
                (select count(*) active_user_count --活跃用户数
                   from %(s_dwd_visqual_probes)s t1
                   left join %(s_dwd_visqual_probes)s t2
                     on t1.row_id = t2.row_id
                    and t2.pt_time = %(ARG_OPTIME_LAST2_MEND)s
                  where t1.pt_time = %(ARG_OPTIME_LASTMONEND)s
                    and t1.online_times - nvl(t2.online_times,0) > 0
                ) t3
            '''% vars()
    QueryExe(hsql,name,dates)
