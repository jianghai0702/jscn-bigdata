...
#-------------------------------------------------------------------------------
# Name:ST不同时段不同网站类型用户
# Pay:ST_DIFF_TIME_URLTYPE_USER_M.py
# Purpose: 
#
# Author:      zhangzc
#
...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_time_urltype_user_m                =    "st_diff_time_urltype_user_m"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#ST不同时段不同网站类型用户
    hsql='''insert overwrite table %(t_st_diff_time_urltype_user_m)s partition(pt_mon = %(ARG_OPTIME_LAST_M)s)
    select
       t1.OWN_CORP_STD_ORG_CODE   --标准归属分公司编码
       ,t1.OWN_CORP_STD_ORG_NAME  --标准归属分公司名称
       ,t4.STD_TIMES_CODE         --标准时段编码      
       ,t4.STD_TIMES_NAME         --标准时段名称      
       ,t3.URL_TYPE               --网址类型          
     ...
     from %(s_dwa_diff_organize_user_m)s t1 
         ...
         on t2.std_online_media_code = t3.STD_URL_CODE
         left join %(s_dwa_diff_times_device_user_d)s t4
         on t2.std_device_cate_code = t4.STD_DEVICE_CATE_CODE
         where t1.pt_mon = %(ARG_OPTIME_LAST_M)s
         and t2.pt_time like '%(ARG_OPTIME_LAST_M)s%%'
         and t4.pt_time like '%(ARG_OPTIME_LAST_M)s%%'
         group by own_corp_std_org_code,OWN_CORP_STD_ORG_NAME,STD_TIMES_CODE,STD_TIMES_NAME,URL_TYPE
     '''% vars()
