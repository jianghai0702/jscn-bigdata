...
## -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name: ST不同上网时长的用户数统计(月)
# Pay: ST_DIFF_ONLINE_LENGTH_USER_M.py
# Purpose:
#
# Author:      liuyf7
#
...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_online_length_user_m     =    "st_diff_online_length_user_m"
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
...
#生成目标表数据
    hsql='''insert overwrite table %(t_st_diff_online_length_user_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
           ...
           inner join
                 (--按上网时长分组，统计用户数
                  select b.std_online_length_code,
                         b.std_online_length_name,
                         c.own_corp_std_org_code,
                         count(distinct a.login_name) user_count
                    from (--计算各用户平均每天上网时长，上网时长/实际上网天数
                          select login_name,
                                 round(sum(case when total_minutes>1440 then 1440 else total_minutes end)/count(distinct pt_time)/60,2) duration 
                            from %(s_dwa_user_online_length_d)s tt
                           where substr(pt_time,1,6) = %(ARG_OPTIME_LAST1_M)s
                           group by login_name
                          ) a
                         ,%(s_dim_std_online_length)s b
                         ,%(S_DWA_FACT_INS_PROD_MON)s c
                   where a.duration <= b.max_value
                     and a.duration > b.min_value
                     and a.login_name = c.login_name
                     and c.is_valid = 1
                     and c.pt_mon = %(ARG_OPTIME_LAST1_M)s
                   group by b.std_online_length_code,b.std_online_length_name,c.own_corp_std_org_code
                 ) t2
