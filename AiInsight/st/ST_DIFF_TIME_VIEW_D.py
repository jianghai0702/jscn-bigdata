...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_time_view_d 				            =    "st_diff_time_view_d"
...
#生成 目标表
    hsql='''insert overwrite table %(t_st_diff_time_view_d)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
              t1. cycle_id,
              ...
              t1.tv_user_count,
              0 as vod_user_count,
              0 as review_user_count,
              0 as  boot_rate,
              case when coalesce(t2.tv_user_count,0)=0 then 0 else round(t1.tv_user_count/t2.tv_user_count,2) end tv_boot_rate,
              0 as vod_boot_rate,
              0 as review_boot_rate,
              current_timestamp as  etl_date
            from %(s_st_tv_diff_time_view_d)s t1
            left outer join %(s_st_tv_total_view_d)s t2 
...
##删除临时表                                               
##===========================================================================
#    hsql='''drop table %(t_st_diff_time_view_d_tmp1)s''' %vars()
#    QueryExe(hsql,name,dates)

#    hsql='''drop table %(t_st_diff_time_view_d_tmp2)s''' %vars()
#    QueryExe(hsql,name,dates)

#    hsql='''drop table %(t_st_diff_time_view_d_tmp3)s''' %vars()
#    QueryExe(hsql,name,dates)
