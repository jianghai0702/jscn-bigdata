...
#-------------------------------------------------------------------------------
# Name: ST不同节目类型点播统计
# Pay: ST_DR_MOVIE_TYPE_SUMMARY.py
# Purpose: 
# 
# Author:      wangxx
# 
...
# 目标表声明
#-------------------------------------------------------------------------------
t_st_dr_movie_type_summary                    =    "st_dr_movie_type_summary"    
...
try:
    ...
    hsql='''
        insert overwrite table %(t_st_dr_movie_type_summary)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
        ...
        INNER  JOIN (select program_name,category1 from %(s_dim_dr_program_label)s lateral view explode(category) mt as category1) AS t2
            ON  t1.movie_name = t2.program_name
        WHERE COALESCE(t1.own_corp_std_org_code,'') <> ''
          AND COALESCE(t2.category1,'') <> ''
        GROUP BY t1.own_corp_std_org_code,t1.own_corp_std_org_name,t2.category1
    '''% vars()
