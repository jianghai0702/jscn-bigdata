...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_st_diff_time_user_m                =    "st_diff_time_user_m"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#补上线时间在前三天的访问数据
    hsql='''insert overwrite table %(t_st_diff_time_user_m)s partition(pt_mon = %(ARG_OPTIME_LAST_M)s)
    select
       t1.own_corp_std_org_code       --标准归属分公司编码
       ,t1.own_corp_std_org_name      --标准归属分公司名称
       ,t3.std_times_code             --标准时段编码      
       ,t3.std_times_name             --标准时段名称      
     ...
     from %(s_dwa_diff_organize_user_m)s t1 
         ...
         left join %(s_dwa_diff_times_device_user_d)s t3
         on t2.std_device_cate_code = t3.STD_DEVICE_CATE_CODE
         where t1.pt_mon = %(ARG_OPTIME_LAST_M)s
         and t2.pt_time like '%(ARG_OPTIME_LAST_M)s%%'
         and t3.pt_time like '%(ARG_OPTIME_LAST_M)s%%'
         group by own_corp_std_org_code,own_corp_std_org_name,std_times_code,std_times_name
     '''% vars()
