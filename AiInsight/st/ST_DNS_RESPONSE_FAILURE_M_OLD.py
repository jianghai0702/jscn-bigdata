...
select 
       ...
       ,from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date  
from (select count(1) as count from %(s_dwd_dns_up_stream)s where substr(pt_time,1,6) = %(ARG_OPTIME_LAST1_M)s and pt_time >20170802) t1 --上行
    ,(select count(1) as count from %(s_dwd_dns_down_stream)s where substr(pt_time,1,6) = %(ARG_OPTIME_LAST1_M)s and pt_time >20170802) t2 --下行
