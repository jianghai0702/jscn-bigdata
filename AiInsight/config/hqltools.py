...
# 执行1条HQL, 没有返回结果, 开启动态分区
def QueryExeNoStrict(hql, name, dates):
    lock_file = join(lpath, name + '_' + dates + '.lock')
    flag = False
    #cur11
    try:
        conn = pyhs2.connect(host=hv_host,port=hv_port,authMechanism=hv_authMechanism,user=hv_user,password=hv_password,database=hv_database)
        cur11 = conn.cursor()
        cur11.execute('set hive.exec.dynamic.partition.mode=nostrict')
        logger.info('Query sql is:\n%s',hql)
        cur11.execute(hql)
        logger.info('Successful implementation of this Sql')
        flag = True
...
def Pama(dicts, dates):
    ...
    #对应输入日期前一天对应的上一年最后一天
    lastdaylastyearendday = datetime.date(lastday.year, 01, 01) - datetime.timedelta(1)
    #对应输入日期前一天对应的上两年最后一天
    lastdaylast2yearendday = datetime.date(lastdaylastyearendday.year, 01, 01) - datetime.timedelta(1)
