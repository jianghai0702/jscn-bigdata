...
dt=`date -d "$arg1 -1 day " +%Y%m%d`
ct=`date -d "$arg1 -0 day " +%Y%m%d`
nt=`date -d "$arg1 +1 day " +%Y%m%d`
echo "dt=${dt}"
echo "ct=${ct}"
echo "nt=${nt}"
echo ">> sh flume_collect.sh ${dt}"
...
# jar包执行时间稍微置后一些
c_hour_delay=`date +%H --date="+1 minute"`
c_minute_delay=`date +%M --date="+1 minute"`
c_second_delay=`date +%S --date="+1 minute"`
