...
def csv_from_excel():
    ...
    tar_dir = "/data/boss_data/stg_rpt_inst_prod/"
    ...
    try:
        ...
        for sheet_index in range(0,count):
          ...
          for row in xrange(sh.nrows):
              #去掉第一页前面两行和最后一页后面两行
              if (sheet_index==0 and row<=1):
                continue
              if (sheet_index==count-1 and row>=sh.nrows-2):
                continue
              ...
              for col in xrange(sh.ncols+1):
                  ...
                  if isInt:
                      ...
                      if isLong:
                          strValue = int(float(strValue))
                  #时间处理
                  if col==12 or col==13 or col==15:
                      if sh.cell(row,col).value=="":
                         strValue = ""
                      else:
                         strValue = datetime(*xldate_as_tuple(sh.cell(row,col).value,0))
