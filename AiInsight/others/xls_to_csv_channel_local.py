...
def csv_from_excel():
    xls = sys.argv[1]
    tar = xls.split('.')[0]
    xls_dir = "/home/ftpuser/tv/tv_channel_program_local/"
    tar_dir = "/data/tv_data/stg_channel_program_local/"
        ...
        for sheet_index in range(0,count):
          ...
          for row in xrange(sh.nrows):
              ...
              for col in xrange(sh.ncols+1):
                  ...
                  if col==sh.ncols:
                      #提取文件名中的城市
                      strValue = re.findall(r'tv_channel_(.*)_\d{8}\.*',xls)[0]
                      #strValue = xls.split('_')[2]
                  else:
                      #取单元格数据
                      if isinstance(sh.cell(row,col).value, unicode):
                          # if sh.cell(row,col).ctype == 3:  # 时间格式
                          #     date = datetime(xldate_as_tuple(sh.cell(row,col).value.encode("utf-8"),0))
                          #     strValue = date.strftime('%H:%M:%S')
                          # else:
                          #     strValue = (str(sh.cell(row,col).value.encode("utf-8")))
                          strValue = (str(sh.cell(row,col).value.encode("utf-8")))
                      else:
                          # if sh.cell(row,col).ctype == 3:  # 时间格式
                          #     date = datetime(xldate_as_tuple(sh.cell(row,col).value,0))
                          #     strValue = date.strftime('%H:%M:%S')
                          # else:
