...
#-------------------------------------------------------------------------------
# Name:DWA各在线媒体用户(日)
# Pay:DWA_DIFF_ONLINE_MEDIA_USER_D.py
# Purpose: 
#
# Author:      liuyf7
#
...
##===========================================================================
    ...
    s_tmp_dwa_diff_online_media_user_d1                =    "s_tmp_dwa_diff_online_media_user_d1"  
    s_tmp_dwa_diff_online_media_user_d2                =    "s_tmp_dwa_diff_online_media_user_d2" 
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_dwa_diff_online_media_user_d                =    "dwa_diff_online_media_user_d"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#用来存放中间结果数据，当日在线记录
    hsql='''create table if not exists %(s_tmp_dwa_diff_online_media_user_d1)s
            (
                acct_start_time    int
                ,acct_stop_time    int
            ...
            stored as rcfile
            LOCATION '%(tmp_path)s/%(s_tmp_dwa_diff_online_media_user_d1)s'
            '''% vars()    
    QueryExe(hsql,name,dates)
    
#插入当日的在线记录
    hsql='''insert overwrite table %(s_tmp_dwa_diff_online_media_user_d1)s
    ...
    hsql='''insert overwrite table %(s_tmp_dwa_diff_online_media_user_d1)s
      select acct_start_time,acct_stop_time,client_ip_address,user_name
        from (select t.*,row_number()over(partition by user_name,acct_start_time order by acct_stop_time desc) rn
                from %(s_tmp_dwa_diff_online_media_user_d1)s t
                 ) t
        where rn = 1
            '''% vars()
    QueryExe(hsql,name,dates)
...
#生成上线时间在当天的访问数据
    hsql='''insert overwrite table %(t_dwa_diff_online_media_user_d)s partition(pt_time=%(ARG_OPTIME_LASTDAY)s,etl_time=%(ARG_OPTIME_LASTDAY)s)
    select
     ...
     from %(s_tmp_dwa_diff_online_media_user_d1)s t1 --在线记录
         ,(select access_time
                 ,source_ip
                 ,domain_name
...
#补上线时间在前一天的访问数据
    hsql='''insert overwrite table %(t_dwa_diff_online_media_user_d)s partition(pt_time=%(ARG_OPTIME_LAST2DAY)s,etl_time=%(ARG_OPTIME_LASTDAY)s)
    select
       t2.domain_name as std_online_media_code
       ,'' as std_online_media_name
       ,t2.std_device_cate_code
...
#补上线时间在前两天的访问数据
    hsql='''insert overwrite table %(t_dwa_diff_online_media_user_d)s partition(pt_time=%(ARG_OPTIME_LAST3DAY)s,etl_time=%(ARG_OPTIME_LASTDAY)s)
    select
       t2.domain_name as std_online_media_code
       ,'' as std_online_media_name
       ,t2.std_device_cate_code
...
#补上线时间在前三天的访问数据
    hsql='''insert overwrite table %(t_dwa_diff_online_media_user_d)s partition(pt_time=%(ARG_OPTIME_LAST4DAY)s,etl_time=%(ARG_OPTIME_LASTDAY)s)
    select
       t2.domain_name as std_online_media_code
       ,'' as std_online_media_name
       ,t2.std_device_cate_code
