...
#生成    当周期的记录
    hsql='''insert overwrite table %(t_dwa_user_network_website_beh_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
             ...
             inner join %(s_dwd_fact_ins_prod)s t6  on trim(t1.prod_inst_id)=trim(t6.prod_inst_id) and t6.pt_mon=%(ARG_OPTIME_LAST1_M)s
             inner join %(s_dwa_cust_feat_dtl_m)s t7 
             on  t6.cust_id  = t7.cust_id
