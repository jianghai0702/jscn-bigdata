...
## -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name:DWA各时段使用各设备的上网用户(日)
# Pay:DWA_DIFF_TIMES_DEVICE_USER_D.py
# Purpose:
#
# Author:      liuyf7
#
...
##===========================================================================
    ...
    s_dwd_nginx_log                 =    "dwd_nginx_log"
    s_dim_std_times                 =    "dim_std_times"
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_dwa_diff_times_device_user_d                =    "dwa_diff_times_device_user_d"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
...
#插入当日各设备的访问用户
    hsql='''insert overwrite table %(t_dwa_diff_times_device_user_d)s partition(pt_time=%(ARG_OPTIME_LASTDAY)s)
    select std_times_code
          ,std_times_name
          ,std_device_cate_code
          ,std_device_cate_name
          ,login_name
      ...
      from (
           select distinct
              ...
              ,t2.std_device_cate_name
              ,t2.remote_addr login_name
            from %(s_dwd_nginx_log)s t2,%(s_dim_std_times)s t3
           where t3.min_value <= hour(t2.time_local)
             and t3.max_value > hour(t2.time_local)
             and t2.pt_time = %(ARG_OPTIME_LASTDAY)s --取当天数据
           ) t
            '''% vars()
