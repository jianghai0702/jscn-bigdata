...
#-------------------------------------------------------------------------------
# Name:DWA用户月全量组织(月)
# Pay:DWA_DIFF_ORGANIZE_USER_M.py
# Purpose: 
#
# Author:      zhangzc
#
...
# 修改日志：
# sunwj 20190617
# dwa_fact_rpt_ins_prod_mon --> DWA_FACT_INS_PROD_MON 20190612 SUNWJ
...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_dwa_diff_organize_user_m                =    "dwa_diff_organize_user_m"    
##===========================================================================       
##创建临时表                                                                       
##===========================================================================
#DWA用户月全量组织
    hsql='''insert overwrite table %(t_dwa_diff_organize_user_m)s partition(pt_mon=%(ARG_OPTIME_LAST_M)s)
    select
         t1.login_name
       ,t1.own_corp_std_org_code    --标准归属分公司编码
       ,t2.std_org_name as own_corp_std_org_name   --标准归属分公司名称      
       ,from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') etl_date  
     from %(s_dwa_fact_ins_prod_mon)s t1 
         left join %(s_dim_std_organize)s t2 --组织表
         on t1.own_corp_std_org_code = t2.std_org_code
         where t1.pt_mon = %(ARG_OPTIME_LAST_M)s
         group by login_name,own_corp_std_org_code,std_org_name  
     '''% vars()
