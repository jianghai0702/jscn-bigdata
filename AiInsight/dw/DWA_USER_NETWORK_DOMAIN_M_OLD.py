...
#-------------------------------------------------------------------------------
# Name:DWA用户上网域名月表
# Pay:DWA_USER_NETWORK_DOMAIN_M.py
# Purpose:
#
# Author:      chens
#
...
#    s_dwd_fact_ins_prod                      =    "dwd_fact_ins_prod"
    s_dwd_fact_ins_prod                      =    "dwa_fact_rpt_ins_prod_mon"
    s_temp_dwa_user_network_domain_m1        =    "temp_dwa_user_network_domain_m1"
    
##===========================================================================       
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_dwa_user_network_domain_m              =    "dwa_user_network_domain_m" 
##===========================================================================       
##创建临时表                                                        
##===========================================================================
#用来存放用户各网站类型各域名的上网次数
    hsql='''create table if not exists %(s_temp_dwa_user_network_domain_m1)s
            (
              prod_inst_id      string,
              website_type      string,
              domain_name       string,
            ...
            stored as orcfile
            LOCATION '%(tmp_path)s/%(s_temp_dwa_user_network_domain_m1)s'
            '''% vars()    
...
##===========================================================================
#生成  临时表1
    hsql='''insert overwrite table %(s_temp_dwa_user_network_domain_m1)s
            select
                t2.prod_inst_id,
                t3.url_type website_type,
                t1.std_online_media_code domain_name,
                count(1) net_num
            ...
            left join %(s_dim_std_url)s t3 on t1.std_online_media_code=t3.std_url_code
            where substr(t1.pt_time,1,6)='%(ARG_OPTIME_LAST1_M)s'
            group by t2.prod_inst_id,t3.url_type,t1.std_online_media_code
         '''% vars()
    QueryExe(hsql,name,dates)    
...
##===========================================================================
#生成    当周期的记录
    hsql='''insert overwrite table %(t_dwa_user_network_domain_m)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select
                t1.prod_inst_id,
                t1.website_type,
                t1.domain_name,
             ...
             from %(s_temp_dwa_user_network_domain_m1)s t1
          '''% vars()    
    QueryExe(hsql,name,dates)
...
##删除临时表                                               
##===========================================================================
#    hsql='''drop table %(s_temp_dwa_user_network_domain_m1)s''' %vars()
#    QueryExe(hsql,name,dates)
