...
##自定义变量声明---目标表声明                                                        
##=========================================================================== 
    t_DWA_USER_TV_PROGRAM_M           =    "DWA_USER_TV_PROGRAM_M"
...
#生成目标表数据
    hsql='''insert overwrite table %(t_DWA_USER_TV_PROGRAM_M)s partition (pt_mon=%(ARG_OPTIME_LAST1_M)s)
              select
                t1.user_id prod_inst_id,
                t1.program_nam PROGRAM_NAME,
                count(1) tv_num,
                sum(t1.uncontinue) tv_time,
                from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') ETL_DATE
                from %(s_dwd_ter_service)s t1
                where t1.pt_mon=%(ARG_OPTIME_LAST1_M)s
                  and t1.program_nam <> '结束'
                  and coalesce(t1.program_nam,'') <> ''
                group by t1.user_id,t1.program_nam
              '''% vars()
