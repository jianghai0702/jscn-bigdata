...
##===========================================================================
    ...
    s_tmp_dwa_tvlive_detail_1 =    "tmp_dwa_tvlive_detail_1"
    s_tmp_dwa_tvlive_detail_2 =    "tmp_dwa_tvlive_detail_2"
##===========================================================================
##自定义变量声明---目标表声明
##===========================================================================
...
# 删除临时表
##===========================================================================
    hsql = '''drop table if exists %(s_tmp_dwa_tvlive_detail)s ''' %vars()
    QueryExe(hsql, name, dates)
    hsql = '''drop table  if exists %(s_tmp_dwa_tvlive_detail_1)s ''' %vars()
    QueryExe(hsql, name, dates)
    hsql = '''drop table  if exists %(s_tmp_dwa_tvlive_detail_2)s ''' %vars()
    QueryExe(hsql, name, dates)
##创建临时表
##===========================================================================
    hsql = '''create table if not exists %(s_tmp_dwa_tvlive_detail)s  (
                ...
                ''' % vars()
    QueryExe(hsql, name, dates)
    hsql = '''create table if not exists %(s_tmp_dwa_tvlive_detail_1)s  (
                 user_id                           String ,
                 cust_id                           String ,
                 sn                                String ,
                 OWN_CORP_STD_ORG_CODE             String ,
                 ...
                 STORED AS RCFILE
                 LOCATION '%(tmp_path)s/%(s_tmp_dwa_tvlive_detail_1)s'
                ''' % vars()
    QueryExe(hsql, name, dates)
    hsql = '''create table if not exists %(s_tmp_dwa_tvlive_detail_2)s  (
                 user_id                           String ,
                 cust_id                           String ,
                 sn                                String ,
                 OWN_CORP_STD_ORG_CODE             String ,
                 ...
                 STORED AS RCFILE
                 LOCATION '%(tmp_path)s/%(s_tmp_dwa_tvlive_detail_2)s'
                ''' % vars()
    QueryExe(hsql, name, dates)
...
# 收视开始时间、结束时间在同一天的数据  匹配最早及最晚节目所有节目单
    ...
    hsql = ''' insert overwrite table %(s_tmp_dwa_tvlive_detail_1)s
               select   t1.user_id
                       ,t1.cust_id
                       ,t1.sn
               ...
               left join %(s_dwd_config_channel)s t2
    ...
    hsql =''' insert overwrite table %(s_tmp_dwa_tvlive_detail_2)s
               select t1.user_id
                       ,t1.cust_id
                       ,t1.sn
                       ,t1.OWN_CORP_STD_ORG_CODE
    ...
    hsql='''insert overwrite table %(s_tmp_dwa_tvlive_detail)s
            ...
            from
              ( 
                select * from %(s_tmp_dwa_tvlive_detail_2)s
                 union all
                 select * from %(s_tmp_dwa_tvlive_detail_1)s
...
# 收视处理
    hsql='''insert overwrite table %(t_dwa_tvlive_detail)s partition(pt_time=%(ARG_OPTIME_LASTDAY)s)            
            select  T.user_id
                   ...
                   ,T.start_time
                   ,T.end_time
                   ,unix_timestamp(T.end_time)-unix_timestamp(T.start_time) as duration
                   ,T.is_local
            from 
            (select  user_id
                    ,cust_id
