...
##===========================================================================   
#在临时表中删除变更数据
    hsql='''insert overwrite table %(t_dim_std_channel_tmp)s
            select
              row_number() over (order by t.channe_name) + if(t3.channel_code_base=0,10000000,t3.channel_code_base) std_channel_code,
            ...
            from(
              ...
              where t2.std_channel_name is null 
             ...
             left outer join (select nvl(max(std_channel_code),0) channel_code_base from %(t_dim_std_channel)s) t3
