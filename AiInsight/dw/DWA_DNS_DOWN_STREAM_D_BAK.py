      ...
      hsql='''insert into table %(s_tmp_dwa_dns_down_stream_d2)s
          select
           ...
           from (select user_name,acct_start_time,acct_stop_time,client_ip_address
                   from %(s_tmp_dwa_dns_down_stream_d1)s
                  where substring(acct_start_time,1,8) = %(ARG_OPTIME_LASTDAY)s) t1 --在线记录
    ...
    for n in range(0,12):
      hsql='''insert into table %(s_tmp_dwa_dns_down_stream_d2)s
          select
           ...
           from (select user_name,acct_start_time,acct_stop_time,client_ip_address
                   from %(s_tmp_dwa_dns_down_stream_d1)s
                  where substring(acct_start_time,1,8) = %(ARG_OPTIME_LAST2DAY)s) t1 --在线记录
           join (select query_time
                       ,user_address
                       ,split(analysis_address, ';')[0] as analysis_address
                       ,answer_code
    ...
    for n in range(0,12):
      hsql='''insert into table %(s_tmp_dwa_dns_down_stream_d2)s
          select
           ...
           from (select user_name,acct_start_time,acct_stop_time,client_ip_address
                   from %(s_tmp_dwa_dns_down_stream_d1)s
                  where substring(acct_start_time,1,8) = %(ARG_OPTIME_LAST3DAY)s) t1 --在线记录
           join (select query_time
                       ,user_address
                       ,split(analysis_address, ';')[0] as analysis_address
                       ,answer_code
    ...
    for n in range(0,12):
      hsql='''insert into table %(s_tmp_dwa_dns_down_stream_d2)s
          select
           ...
           from (select user_name,acct_start_time,acct_stop_time,client_ip_address
                   from %(s_tmp_dwa_dns_down_stream_d1)s
                  where substring(acct_start_time,1,8) = %(ARG_OPTIME_LAST4DAY)s) t1 --在线记录
           join (select query_time
                       ,user_address
                       ,split(analysis_address, ';')[0] as analysis_address
                       ,answer_code
