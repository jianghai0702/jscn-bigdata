...
## -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name:DWA流失用户月表 (经分 boss1 + boss2）
# Pay:DWA_LOST_USERS_MON.py
# Purpose:
#
# Author:      wangxx
#
...
##自定义变量声明---目标表声明
##===========================================================================
    t_dwa_lost_users_mon       =    "dwa_lost_users_mon"
##===========================================================================
##创建临时表
##===========================================================================
##===========================================================================
...
##===========================================================================
# 生成目标数据
    hsql='''insert into table %(t_dwa_lost_users_mon)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
            select 
              t1.prod_inst_id , 
              t1.cust_id , 
              t1.acct_id , 
              ...
              t1.tv_pay_status , 
              t1.is_ins , 
              t1.data_source_cd ,  -- 表示数据来源是BOSS1 还是BOSS2
              %(ARG_OPTIME_LAST1_M)s as etl_cycle,
              current_timestamp  as     etl_date
             from %(s_dwa_fact_ins_prod_mon)s t1  -- 上月用户
             where t1.pt_mon = %(ARG_OPTIME_LAST2_M)s
               and t1.is_valid1 = 1 
               and not exists (select 1 
                                 from %(s_dwa_fact_ins_prod_mon)s t2  -- 本月用户
