...
#-------------------------------------------------------------------------------
# Name: DWA产品退订累积表
# Pay: DWA_FACT_INS_SRVPKG_CANCEL.py
# Purpose: 每月boss1、boss2用户产品退订信息
# 
# Author:      wangxx
# 
# Created:     2018-10-31
...
# 目标表声明
#-------------------------------------------------------------------------------
t_dwa_fact_ins_srvpkg_cancel            =    "dwa_fact_ins_srvpkg_cancel"
...
try:
    ...
    hsql=r'''
        INSERT overwrite table %(t_dwa_fact_ins_srvpkg_cancel)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
        SELECT 
           t1.SRVPKG_INST_ID                             ,
           t1.OFFER_INST_ID                              ,
           t1.OFFER_ID             ,
        ...
        LEFT JOIN %(s_stg_fact_ins_srvpkg)s t2  -- 判断是否上月退订
    ...
    hsql=r'''
        INSERT INTO table %(t_dwa_fact_ins_srvpkg_cancel)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
        SELECT 
           t1.SRVPKG_INST_ID                             ,
           t1.OFFER_INST_ID                              ,
           t1.OFFER_ID             ,
        ...
        LEFT JOIN %(s_stg_fact_ins_srvpkg_b2)s t2  -- 判断是否上月退订
            ON t1.srvpkg_inst_id = t2.srvpkg_inst_id
           AND t2.STATE = 1 -- 正常订购状态
           AND t2.pt_mon = %(ARG_OPTIME_LAST1_M)s
        WHERE t1.pt_mon = %(ARG_OPTIME_LAST2_M)s
