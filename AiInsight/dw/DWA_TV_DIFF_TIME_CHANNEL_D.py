...
# -*-coding:utf-8 -*-
#-------------------------------------------------------------------------------
# Name: DWA直播各时段各频道收视统计(日)
# Pay: DWA_TV_DIFF_TIME_CHANNEL_D.py
...
# 目标表声明
#-------------------------------------------------------------------------------
t_dwa_tv_diff_time_channel_d                    =    "dwa_tv_diff_time_channel_d"
...
try:
    ...
    hsql=r'''
        insert overwrite table %(t_dwa_tv_diff_time_channel_d)s partition(pt_mon=%(ARG_OPTIME_LAST1_M)s)
                    select
                     ...
                     where t1.pt_mon='%(ARG_OPTIME_LAST1_M)s'
                           ...
                           and HOUR(t1.end_time) * 60 + minute(t1.end_time)   >= CAST(t2.min_value AS decimal) * 60
                           AND t1.tv_day BETWEEN '%(ARG_OPTIME_LASTMON01)s' AND '%(ARG_OPTIME_LASTMONEND)s'
                    group by t1.tv_day,t1.own_corp_std_org_code,t3.std_org_name,t2.std_times_code,t2.std_times_name,t1.channel_name,CAST(t2.max_value - t2.min_value AS INT) * 60
    ...
    hsql=r'''
        insert into table %(t_dwa_tv_diff_time_channel_d)s partition (pt_mon='%(ARG_OPTIME_LAST1_M)s')
                    select t1.cycle_id, 
                             ...
                             current_timestamp as etl_date
                    from %(t_dwa_tv_diff_time_channel_d)s as t1
                   ...
                   where t1.pt_mon = '%(ARG_OPTIME_LAST1_M)s'
                    GROUP BY cycle_id,t3.std_org_code,t3.std_org_name,t1.std_times_code,t1.std_times_name,t1.std_channel_code,t1.std_channel_name,t1.std_times_duration
                    grouping sets ( (cycle_id,t3.std_org_code,t3.std_org_name,t1.std_times_code,t1.std_times_name,t1.std_channel_code,t1.std_channel_name,t1.std_times_duration) ,
                                           (cycle_id,t1.std_times_code,t1.std_times_name,t1.std_channel_code,t1.std_channel_name,t1.std_times_duration) )
