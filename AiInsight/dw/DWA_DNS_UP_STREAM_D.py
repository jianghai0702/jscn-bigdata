...
##===========================================================================    
    ...
    hsql='''insert into table %(s_tmp_dwa_dns_up_stream_d3)s
      select
       ...
       from %(s_tmp_dwa_dns_up_stream_d1)s t1 --在线记录
           ,(select * from %(s_dwd_dns_up_stream)s where pt_time=%(ARG_OPTIME_LASTDAY)s
      )t2 --上行数据
    ...
    hsql='''insert into table %(s_tmp_dwa_dns_up_stream_d3)s
      select
         ...
         ,count(1)
       from %(s_tmp_dwa_dns_up_stream_d1)s t1 --在线记录
           ,(select * from %(s_dwd_dns_up_stream)s where pt_time=%(ARG_OPTIME_LAST2DAY)s 
            )t2 --上行数据
      where t2.source_adress = t1.client_ip_address
        and t1.acct_start_time <= t2.visit_time
        and t1.acct_stop_time >= t2.visit_time
    ...
    hsql='''insert into table %(s_tmp_dwa_dns_up_stream_d3)s
      select
         ...
         ,count(1)
       from %(s_tmp_dwa_dns_up_stream_d1)s t1 --在线记录
           ,(select * from %(s_dwd_dns_up_stream)s where pt_time=%(ARG_OPTIME_LAST3DAY)s
            )t2 --上行数据
      where t2.source_adress = t1.client_ip_address
        and t1.acct_start_time <= t2.visit_time
        and t1.acct_stop_time >= t2.visit_time
    ...
    hsql='''insert into table %(s_tmp_dwa_dns_up_stream_d3)s
      select
         ...
         ,count(1)
       from %(s_tmp_dwa_dns_up_stream_d1)s t1 --在线记录
           ,(select * from %(s_dwd_dns_up_stream)s where pt_time=%(ARG_OPTIME_LAST4DAY)s 
             )t2 --上行数据
      where t2.source_adress = t1.client_ip_address
        and t1.acct_start_time <= t2.visit_time
        and t1.acct_stop_time >= t2.visit_time
